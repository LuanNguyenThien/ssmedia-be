...
class RedisManager {
  ...
  private static bclient: Redis;
  private static redisUrl = config.REDIS_HOST || 'redis://localhost:6379';

  static getClient(): Redis {
    if (!this.client) {
      this.client = new Redis(this.redisUrl, {
        enableReadyCheck: true, // `client` có thể giữ mặc định để kiểm tra kết nối.
      });
    }
    return this.client;
  }
  ...
  static getSubscriber(): Redis {
    if (!this.subscriber) {
      this.subscriber = new Redis(this.redisUrl, {
        enableReadyCheck: false, // Để tránh lỗi khi dùng làm subscriber.
        maxRetriesPerRequest: null,
      });
    }
  ...
  static getBClient(): Redis {
    if (!this.bclient) {
      this.bclient = new Redis(this.redisUrl, {
        enableReadyCheck: false,
        maxRetriesPerRequest: null,
      });
    }
