...
export class MessageCache extends BaseCache {
  ...
  public async getChatMessagesFromCache(senderId: string, receiverId: string, isGroupChat: false): Promise<IMessageData[]> {
    try {
      if (!this.client.isOpen) {
        await this.client.connect();
      }
  ...
  public async updateMessageReaction(
    ...
    try {
      ...
      if (parsedMessage) {
        ...
        const existingReactionIndex = findIndex(parsedMessage.reaction, 
          (r: IReaction) => r.senderName === senderName && r.type === reaction);
        
        // Remove any existing reactions from this sender
        remove(parsedMessage.reaction, (r: IReaction) => r.senderName === senderName);
        
        // If adding a reaction and it's not the same as an existing one, add it
        if (type === 'add' && existingReactionIndex === -1) {
          ...
          // If removing or toggling the same reaction, just save with the reaction removed
          await this.client.lSet(`messages:${conversationId}`, messageIndex, JSON.stringify(parsedMessage));
        }
      }
