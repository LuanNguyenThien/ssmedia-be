...
class CommentService {
  public async addCommentToDB(commentData: ICommentJob): Promise<void> {
    ...
    try {
      ...
      const postUpdated: IPostDocument | null = await PostModel.findByIdAndUpdate(
        postId,
        { $inc: { commentsCount: 1 } },
        { new: true }
      );
      
      console.log('Post update result:', postUpdated ? 
        `Updated post ${postId}, new commentsCount: ${postUpdated.commentsCount}` : 
