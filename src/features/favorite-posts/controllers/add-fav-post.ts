...
export class Add {
  ...
  public async favoritePost(req: Request, res: Response): Promise<void> {
    ...
    if (post?.favoritedBy?.includes(userId) ?? false) {
      ...
      await PostModel.updateOne({ _id: postId }, { $pull: { favoritedBy: userId } });
    } else {
      // Nếu chưa tồn tại, thêm userId vào mảng favoritedBy
      await PostModel.updateOne({ _id: postId }, { $addToSet: { favoritedBy: userId } });
    }
    res.status(HTTP_STATUS.CREATED).json({ message: 'Successfully' });
  }
}
